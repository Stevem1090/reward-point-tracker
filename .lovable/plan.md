...
- If a repeating task isn't ticked, its reminder fires only once. It stays in the list showing the missed date (in red, as overdue) until ticked.
- When the next repeat day arrives, a missed task resets to show the new date and the normal reminder fires again at that time.
...
- `send-push-notification` task-reminder job (already every minute):
  ...
  - Unticked repeating tasks keep their missed `due_at` (already notified once). When London midnight of the next occurrence day arrives, set `due_at` to that occurrence (trigger clears `notified_at`) so it shows the new date and notifies at its time.
